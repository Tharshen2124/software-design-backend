...
class MaintenanceProjectData:
    def __init__(self, title, description, status, complaint_ids, maintenance_company_id, image_url, maintenance_project_id):
        self.title = title
    ...
    def get_data_for_update(request):
        try:
            ...
            if not project_image:
                return MaintenanceProjectData(None, None, status_enum, None, None, None, maintenance_project_id)
            else:
                project_image = request.FILES.get("project_image")
                cloudinary_client = CloudinaryClient()
                image_url = (cloudinary_client.upload(project_image)).get('secure_url')
                return MaintenanceProjectData(None, None, status_enum, None, None, image_url, maintenance_project_id)
